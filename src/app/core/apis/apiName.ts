export class apiName{
    public static GetAllProducts = 'GetAllProducts';
    public static CreateProduct  = 'CreateProduct';
    public static UpdateProduct = 'UpdateProduct'
    public static DeleteProductById = 'DeleteProductById';
    public static GetAllProductsByCategoryId = 'GetAllProductsByCategoryId'
    public static GetProductById = 'GetProductById';
    public static GetAllCategory = 'GetAllCategory'
    public static CreateNewCategory = 'CreateNewCategory';
    public static DeleteCategoryById = 'DeleteCategoryById';
    public static CreateNewOffer = 'CreateNewOffer';
    public static GetAllOffers = 'GetAllOffers';
    

}